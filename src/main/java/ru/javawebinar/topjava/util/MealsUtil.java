...
public class MealsUtil {
    ...
    public static final List<User> USERS = Arrays.asList(
            ...
            new User(null, "danil", "d@mail.ru", "12345", 1000, true, EnumSet.of(Role.ROLE_USER))
    );
    ...
    public static List<MealWithExceed> getFilteredWithExceeded(Collection<Meal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        return meals.stream().collect(Collectors.groupingBy(Meal::getDate)).values().stream()
                .flatMap(mealsByDay -> {
                    boolean exceeded = mealsByDay.stream().mapToInt(Meal::getCalories).sum() > caloriesPerDay;
                    return mealsByDay.stream().filter(m -> DateTimeUtil.isBetween(m.getTime(), startTime, endTime))
                    .map(m -> createWithExceed(m, exceeded)); }).collect(Collectors.toList());
    }
